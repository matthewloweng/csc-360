...
void kosmos_init() {
    // need to initialize the semaphores
    sem_init(&mutex, 0, 1);
    sem_init(&wait_c, 0, 2);
    sem_init(&wait_h, 0, 1);
    sem_init(&staging_area, 0, 1);

    // initialize the number of free atoms
    num_free_c = 0;
    num_free_h = 0;

    //initialize combining names
    combining_c1 = 0;
...
void *h_ready( void *arg )
{
    ...
    if (num_free_c >= 2) {
        strncpy(combiner, name, MAX_ATOM_NAME_LEN);
...
void make_radical(int c1, int c2, int h, char *maker)
{
    ...
    fprintf(stdout, "A ethynyl radical was made: c%03d  c%03d  h%03d\n",
        c1, c2, h);
#endif
    // lock the staging area
    sem_wait(&staging_area);
    
    radicals++;

    // signal the waiting atoms
    sem_post(&wait_c);
    sem_post(&wait_c);
    sem_post(&wait_h);

    // unlock the staging area
...
void wait_to_terminate(int expected_num_radicals) {
    if (expected_num_radicals == radicals) {   
